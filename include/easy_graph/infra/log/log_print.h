...
#include "easy_graph/infra/log/base_name.h"
#include "easy_graph/infra/log/log_level.h"
#include <type_traits>
#include <iostream>
...
namespace detail {
	...
	void log_print(LogLevel level, const char(&file)[Len], unsigned int line, const char* fmt, TS && ...ts) {
		auto msg = std::string("[") + to_string(level) + "]: " + base_name_of(file) + ":" + std::to_string(line) + ": ";
...
do {													\
	if (::EG_NS::level_enabled(level)) {			    \
		::EG_NS::detail::log_print(level,  __FILE__, __LINE__, fmt, ##__VA_ARGS__);\
	}										            \
} while(0)
